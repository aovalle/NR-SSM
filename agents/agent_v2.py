...
class AgentV2(object):
    def __init__(self, env, test_env, args):
        ...
        self.rep_optim = torch.optim.Adam([{'params': get_parameters(self.rep_nets), 'lr': args.lr_wm},
                                       {'params': get_parameters(self.nce_nets), 'lr': args.lr_nce}])  # , eps=args.epsilon)
                                       #{'params': self.classifier.parameters(), 'lr': args.lr_nce}])  # , eps=args.epsilon)
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_train_epochs):
            ...
            # Learn World Model
            self.learn_perception(obs, neg_obs, onehot_actions, rewards, nonterminals, noninit)
    ...
    def learn_perception(self, obs, neg_obs, actions, rewards, nonterminals, noninit):
        # o_t, a_t - 1 = 0, r_t, d_t, i_t(starting state)
        # Encode obs o_t:T+1
        ...
        fg_t_next = embobs['out'][1:]  # (seq, batch, embedding size)  (t:T)
        ...
        prior_trans, post_trans = self.rssm.get_transitions(fg_t, actions[:-1], noninit[:-1])
        # -> (seq, batch, s + h dim)
        ...
        if self.args.nce_method == 'nce-glt':
            ...
            _, post_trans_l = self.rssm.get_transitions_flocal(fl_t, actions[:-1], noninit[:-1])
        ...
        # Backprop
        # self._backprop(self.wm_optim, loss, self.wm_nets + [self.classifier])
        self._backprop(self.rep_optim, loss, self.rep_nets + self.nce_nets)

        # DECODER To check what it's capturing
        # with no_grad_in(self.wm_nets, self.classifier):
        # with no_grad_in(self.rep_nets, self.classifier):
