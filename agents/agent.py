...
from buffers.trajectory_buffer_negatives import TrajectoryBuffer
...
class Agent(object):
    def __init__(self, env, test_env, args):
        ...
        self.wm_nets = [self.rssm, self.embedder]
    ...
    def train(self):
        ...
        while self.step <= self.args.n_seed_steps + self.args.n_steps and self.learning_episodes < self.args.n_episodes:
            ...
            while not done:
                ...
                self.buffer.append(obs, action, reward, done)   # o_t, a_t, r_t+1, d_t+1
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_train_epochs):
            # Draw sequence chunks {(o_t:T+1, a_t:T+1, r_t+1:T+2, notdone_t+1:T+2} ~ D
            obs, neg_obs, actions, rewards, nonterminals = self.buffer.sample()
            onehot_actions = one_hot(actions.squeeze(-1).long(), num_classes=self.action_space).float()
            # Learn World Model
            self.learn_perception(obs, neg_obs, onehot_actions, rewards, nonterminals)
    ...
    def learn_perception(self, obs, neg_obs, actions, rewards, nonterminals):
        # We have sequence chunks {(o_t:T+1, a_t:T+1, r_t+1:T+2, notdone_t+1:T+2} ~ D
        # Encode obs o_t+1:T+1 (seq len, batch, embedding size)
        embobs = self.embedder(obs[1:], fmaps=True)
        f_t = embobs['out'][:-1]    # (seq, batch, embedding size)  (t+1:T)
        f_t_next = embobs['f5'][1:] # (seq, batch, 6,6,128)         (t+2:T+1)
        f_neg = self.embedder(neg_obs[1:-1], fmaps=True)['f5']  # (seq, batch, 6,6,128) (len |T-1|)

        #NOTE:
        # 1 get x = {s,h,a} representations from enc(o)
        # 2 peform I(x;x') where x' = {enc(o'),r'}

        # o_t+1:T, a_t:T-1, d_t+1:T -> Obtain h_t+1:T, s_t+1:T
        prior_trans, post_trans = self.rssm.get_transitions(f_t, actions[:-2], nonterminals[:-2])

        # -> (seq, batch, s + h dim) z_t+1:T
        latent = torch.cat((post_trans['h'], post_trans['s']), dim=-1)  # (t+1:T)
        nce_loss = self._nce_loss(latent, f_t_next, f_neg)

        # Get predictive distributions p(.|z_t+1:T)
        # z_t+1:T-1 #note: double cjeck indidces
        # reward_dist = self.reward_model(latent)        # p(r_t+1|s_t,h_t)
        # nonterm_dist = self.discount_model(latent)

        # Losses
        # reward_loss = self._reward_loss(reward_dist, rewards[1:-1])           # r_t+2:T+1
        # nonterm_loss = self._pcont_loss(nonterm_dist, nonterminals[1:-1])     # d_T+2:T+1
