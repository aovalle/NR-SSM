...
class AgentDQN2_GRAD(object):
    def __init__(self, env, test_env, args):
        ...
        if args.env_seed is not None:
            ...
            self.test_env.seed(2 ** 31 - 1 - args.env_seed)
        ...
        if args.nce_method == 'ince-ggt':
            self.classifier = torch.nn.Linear(args.det_size + args.stoch_size + self.action_space, args.embedding_nce).to(device)
        if args.nce_method in ['ince-glt', 'ince-gl-ll']:
            self.classifier = torch.nn.Linear(args.det_size + args.stoch_size + self.action_space, self.embedder.local_layer_depth).to(device)
            if args.nce_method == 'ince-gl-ll':
                self.classifier2 = torch.nn.Linear(args.det_size + args.stoch_size + self.action_space, self.embedder.local_layer_depth).to(device)
                self.nce_nets.append(self.classifier2)
        elif args.nce_method == 'nce-glt':
            # Input 1: latent, Input 2: local features
            self.classifier = BilinearClassifier(args.det_size + args.stoch_size + self.action_space, self.embedder.local_layer_depth).to(device)
        ...
        self.rep_nets = [self.rssm, self.embedder]#, self.reward_model, self.discount_model]
        self.dec_nets = [self.decoder]
        self.rep_optim = torch.optim.Adam([{'params': get_parameters(self.rep_nets), 'lr': args.lr_wm},
                                       {'params': get_parameters(self.nce_nets), 'lr': args.lr_nce},
                                       {'params': self.value_model.parameters(), 'lr': args.lr_value}])
        self.dec_optim = torch.optim.Adam([{'params': get_parameters(self.dec_nets), 'lr': args.lr_wm}])
    ...
    def decide_action(self, latent):
        if self.args.agent in ['q_rhe', 'q_rhe2', 'q_rhe2_grad']:
    ...
    def learn_perception(self, obs, neg_obs, actions, onehot_actions, rewards, nonterminals, noninit):
        ...
        if self.args.nce_method == 'nce-glt':
            nce_loss = self._nce_loss(latent, onehot_actions[1:], fl_t_next, fl_neg)
        elif self.args.nce_method == 'ince-ggt':
            nce_loss = self._infonce_loss(latent, onehot_actions[1:], fg_t_next)
        elif self.args.nce_method == 'ince-glt':
            nce_loss = self._infonce_loss(latent, onehot_actions[1:], fl_t_next)
            ...
            nce_loss = self._infonce_loss(latent, onehot_actions[1:], fl_t_next, latent_l)
        ...
        loss = nce_loss + kl_loss


        value_loss = self.learn_control(latent, actions[:-1], rewards[:-1], nonterminals[:-1])

        # Backprop
        #self._backprop(self.rep_optim, loss, self.rep_nets + self.nce_nets)
        self._backprop(self.rep_optim, loss + value_loss, self.rep_nets + self.nce_nets + [self.value_model])
        ...
        with no_grad_in(self.rep_nets, self.nce_nets):
            obs_dist = self.decoder(latent.detach())    # z_t:T
            reconstruction = self._obs_loss(obs_dist, obs[:-1].detach())    # o_t:T
            #reward_dist = self.reward_model(latent.detach(), onehot_actions[1:])  # p(r_t+1|s_t,h_t,a_t) -> p(r_t+1|s_t, a_t, s_t-1, a_t-1, h_t-1:)
            #reward_loss = self._reward_loss(reward_dist, rewards[1:])  # r_t+1:T
            #nonterm_dist = self.discount_model(latent.detach(), onehot_actions[1:])
            ...
            self._backprop(self.dec_optim, loss_dec, self.dec_nets)

        #return loss, latent


    def _nce_loss(self, f_t, actions, f_t_next, f_neg):
        # Concat f_t, a_t
        f_t = torch.cat([f_t, actions], dim=-1)
    ...
    def _infonce_loss(self, f_tg, actions, f_t_next, f_tl=None):
        # Concat f_t, a_t
        f_tg = torch.cat([f_tg, actions], dim=-1)   # (seq len, batch, *)
        f_tg = f_tg.reshape(-1, f_tg.shape[-1])     # (seq len*batch, *)
        N = f_tg.size(0) # batch size
        # N x N
        if self.args.nce_method == 'ince-ggt':                      # f_tg, f_tg_next
            f_t_next = f_t_next.reshape(-1, f_t_next.shape[-1])
            scores = torch.matmul(self.classifier(f_tg), f_t_next.t())  # (seq len*batch, seq len*batch)
            ...
            if self.args.nce_method == 'ince-gl-ll':                # f_tl, ft_l_next
                actions = actions.unsqueeze(2).unsqueeze(2).expand(f_tl.size(0), f_tl.size(1), f_tl.size(2), f_tl.size(3), -1)
                f_tl = torch.cat([f_tl, actions], dim=-1)
