...
class AgentDQN2(object):
    ...
    def decide_action(self, latent):
        if self.args.agent in ['q_rhe', 'q_rhe2']:
    ...
    def learn_perception(self, obs, neg_obs, actions, onehot_actions, rewards, nonterminals, noninit):
        ...
        with no_grad_in(self.rep_nets, self.nce_nets):
            ...
            # reward_dist = self.reward_model(latent.detach())  # p(r_t|s_t,h_t) -> p(r_t|s_t, s_t-1, a_t-1, h_t-1)
            ...
            # nonterm_loss = self._pcont_loss(nonterm_dist, nonterminals[:-1])  # d_t:T-1
    ...
    def _infonce_loss(self, f_tg, actions, f_t_next, f_tl=None):
        # Concat f_t, a_t
        f_tg = torch.cat([f_tg, actions], dim=-1)
